        ...
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item">
                    <a href="">Payroll Configurations</a>
                </div>
            </div>
        </div>
        <div class="section">
                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                            $currentSalary = $employee->employeeSalaries->where('is_current',
                                            true)->first();
                                            @endphp
                                            @if($currentSalary && $currentSalary->ctc !== null)
                                            {{ $employee->company->default_currency ?? '₹' }}
                                            {{ number_format($currentSalary->ctc, 2) }}
                                            @if($currentSalary->effective_from)
                                            <br><small class="text-muted">From:
                                                {{ $currentSalary->effective_from->format('d M, Y') }}</small>
                                            @endif
                                            @else
                                        ...
                                        <td>
                                            @if(\App\Models\User::hasAccess('admin/payroll/employee-configurations/{encryptedId}/edit', true))
                                            <a href="{{ route('admin.payroll.employee-configurations.edit', \Crypt::encrypt($employee->id)) }}"
