...
protected function isWeekend($date)
{
    ...
    public function adminStore(Request $request)
    {
        ...
        // Check leave balance using LeaveService
        $leaveService = new LeaveService();
        $leaveBalanceCheck = $leaveService->checkLeaveBalance(
            $employee,
            $leaveType,
            $totalWorkingDays,
            $startDate,
        ...
        if (!$leaveBalanceCheck['success']) {
            return redirect()->back()
                ->with('error', $leaveBalanceCheck['message'])
                ->withInput();
        }
        ...
        if ($request->status === 'approved') {
            ...
            if ($leaveBalance) {
                // Deduct from remaining_days instead of incrementing used_days
                // to properly support the new carry forward logic
                $newRemaining = $leaveBalance->remaining_days - $totalWorkingDays;
                $leaveBalance->update([
                    'remaining_days' => $newRemaining,
                    'used_days' => $leaveBalance->used_days + $totalWorkingDays,
                ]);
            }
        }
    ...
    public function approve(Request $request, LeaveRequest $leaveRequest)
    {
        ...
        if ($leaveBalance) {
            // Deduct from remaining_days instead of incrementing used_days
            // to properly support the new carry forward logic
            $newRemaining = $leaveBalance->remaining_days - $workingDaysCount;
            $leaveBalance->update([
                'remaining_days' => $newRemaining,
